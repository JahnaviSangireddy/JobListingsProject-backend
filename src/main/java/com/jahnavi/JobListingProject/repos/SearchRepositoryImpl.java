package com.jahnavi.JobListingProject.repos;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;


import org.springframework.beans.factory.annotation.Autowired;

import org.springframework.stereotype.Component;

import com.jahnavi.JobListingProject.entities.JobPost;


@Component
public class SearchRepositoryImpl implements SearchRepository {

//	 @Autowired
	// MongoClient client;

//	@Autowired
	//MongoConverter converter;
	@Override
	public List<JobPost> findByText(String text) {
		
		 final List<JobPost> posts = new ArrayList<>();

			/*
			 * MongoDatabase database = client.getDatabase("jahnavi");
			 * MongoCollection<Document> collection = database.getCollection("JobListings");
			 * 
			 * AggregateIterable<Document> result = collection.aggregate(Arrays.asList(new
			 * Document("$search", new Document("text", new Document("query", text)
			 * .append("path", Arrays.asList("techs", "desc", "profile")))), new
			 * Document("$sort", new Document("exp", 1L)), new Document("$limit", 5L)));
			 * 
			 * result.forEach(doc -> posts.add(converter.read(JobPost.class,doc)));
			 */

	        return posts;
	    }

}
